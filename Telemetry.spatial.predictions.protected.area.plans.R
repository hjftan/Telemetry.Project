...
load("F:/Project.Caspian.Hardrive/RScripts/Telemetry.Work/Telemetry.fit.inla.model.RData")
...
# generally, our MPA plans are designed to maximize for areas which are predicted to have higher foraging
#intensity and minimize for areas with high vessel density
#the broad idea of this research is to identify areas which are feasible and inexpensive to protect 
#but offer good foraging habitats for seals
...
read.ras <- function(){
  for ( i in 1:12){
    #ras <- raster(paste0("F:/Project.Caspian.Hardrive/RScripts/Telemetry.Work/vessel.rasters/season.num.ves.", i, ".grd"))
    #assign(paste0("season.num.ves.", i), ras, envir = globalenv())
    ras <- raster(paste0("F:/Project.Caspian.Hardrive/RScripts/Telemetry.Work/vessel.rasters/season.num.ves.", i, "new.grd"))
    assign(paste0("season.num.ves.", i), ras, envir = globalenv())
  }
} 
read.ras() 
...
SST <- c(list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2008", full.names = T, recursive = T),
         list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2009", full.names = T, recursive = T),
         list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2010", full.names = T, recursive = T),
         list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2011", full.names = T, recursive = T),
         list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2012", full.names = T, recursive = T),
         #list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2013", full.names = T, recursive = T),
         list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2016", full.names = T, recursive = T),
         list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2017", full.names = T, recursive = T))#,
#list.files("F:/Project.Caspian.Hardrive/Environmental.Data/SST.MUR.NEW/2018", full.names = T, recursive = T))
...
summary.output.paplan <- lapply(season.split, function(x){
  ...
  #sum(st_length(inside.1)) + sum(st_length(outside.1)) # some small overlap of lines clearly, from touiching the intersecting polygon
  #remember none of the stored values are recalculated
  #although speed should be fine to use as its our best guess for any pooint along any of the lines, 
  #split or otherwise
  #estimating impact of vessel speed restricitons would be itnertesting but would require quite abit of thinking
  #still how best to do this
...
#save.image("F:/Project.Caspian.Hardrive/RScripts/Telemetry.Work/Telemetry.spatial.predictions.protected.areas.RData")
